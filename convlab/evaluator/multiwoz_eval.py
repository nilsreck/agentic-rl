...
class MultiWozEvaluator(Evaluator):
    ...
    def add_sys_da(self, da_turn, belief_state=None):
        ...
        for intent, domain, slot, value in da_turn:
            if intent.lower() == "book" and not value:
                ref = [
                    _value
                    for _intent, _domain, _slot, _value in da_turn
                    if _domain == domain
                    and _intent.lower() == "inform"
                    and _slot.lower() == "ref"
                ]
        ...
        for intent, domain, slot, value in da_turn:
            ...
            if intent.lower() == "book":
                ...
                if domain.lower() == "taxi":
                    ...
                    if (
                        not self.booked[domain]
                        and re.match(r"^\d{8}$", value)
                        and len(self.dbs[domain]) > int(value)
                    ):
    ...
    def task_success(self, ref2goal=True):
        ...
        if (
            ...
            self.success = 1
            self.success_strict = (
                1 if (book_constraint_sess == 1 or book_constraint_sess is None) else 0
            )
            return (
                self.success if not self.check_book_constraints else self.success_strict
            )
        else:
            self.complete = (
                1
                if booking_done and (inform_sess[1] == 1 or inform_sess[1] is None)
                else 0
            )
            self.success = 0
            self.success_strict = 0
    ...
    def domain_success(self, domain, ref2goal=True):
        ...
        if (
            ...
            domain_strict_success = (
                1 if (book_constraints == 1 or book_constraints is None) else 0
            )
            return (
                domain_success
                if not self.check_book_constraints
                else domain_strict_success
            )
        else:
            return 0
    ...
    def _final_goal_analyze_domain(self, domain):
        ...
        if "info" in dom_goal_dict:
            ...
            info_constraints = []
        query_result = self.database.query(domain, info_constraints + reqt_constraints)
        if not query_result:
            mismatch += 1
    ...
    def get_reward(self, terminated=False):
        ...
        if terminated:
            ...
            if self.check_domain_success and not self.task_success():
                if (
                    self.cur_domain
                    and self.domain_success(self.cur_domain)
                    and self.cur_domain not in self.successful_domains
                ):
    ...
    def update_goal(self, goal, system_action):
        for intent, domain, slot, val in system_action:
            ...
            if slot in reverse_da_slot_name_map:
                ...
                domain in reverse_da_slot_name_map
                and slot in reverse_da_slot_name_map[domain]
            ):
            ...
            if intent.lower() in ["inform", "recommend"]:
                if domain.lower() in goal:
                    if "reqt" in goal[domain.lower()]:
                        if (
                            REF_SYS_DA_M.get(domain.lower(), {}).get(
                                slot.lower(), slot.lower()
                            )
