...
class UserPolicyAgendaMultiWoz(Policy):
    ...
    def init_session(self, goal=None):
        ...
        # INFO: This is the problematic line with custom goals
    ...
    def _normalize_value(cls, domain, intent, slot, value):
        ...
        if value not in value_list:
            ...
            if value in ["none"]:
                logging.debug(
                    "Value [none] invalid! (Lexicalisation Error) (slot: %s domain: %s)"
                    % (slot, domain)
                )
                return "none"
            if slot in ["phone"]:
    ...
    def __init__(self, goal: Goal):
        ...
        def random_sample(data, minimum=0, maximum=1000):
            return random.sample(
                sorted(data),
                random.randint(min(len(data), minimum), min(len(data), maximum)),
            )
