...
class GoalGenerator:
    ...
    def __init__(self,
                 goal_model_path=os.path.join(
                     get_root_path(), 'data/multiwoz/goal/new_goal_model_restaurant_hotel_only.pkl'),
                 corpus_path=os.path.join(get_root_path(), 'data/multiwoz/train.json.zip'),
                 boldify=False,
                 sample_info_from_trainset=True,
                 sample_reqt_from_trainset=False,
                 seed = 12412, domain_ordering_dist=None):
        """Args:
                    ...
                    if slot in state[book]:
                        if slot == 'people':
                            ...
                            if same_people_domain is None:
                                slot_strings.append(f'for {self.boldify(state[book][slot])} people')
                            else:
                                slot_strings.append(self.boldify(
                                    f'for the same group of people as the {same_people_domain} booking'))
                        elif slot == 'time':
                            slot_strings.append(f'at {self.boldify(state[book][slot])}')
                        elif slot == 'day':
                            same_people_domain = get_same_people_domain(
                                user_goal, dom, slot)
                            if same_people_domain is None:
                                slot_strings.append(f'on {self.boldify(state[book][slot])}')
                            else:
                                slot_strings.append(
                                    self.boldify(f'on the same day as the {same_people_domain} booking'))
                        elif slot == 'stay':
                            slot_strings.append(f'for {self.boldify(state[book][slot])} nights')
                        del state[book][slot]
