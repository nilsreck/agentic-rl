...
class Analyzer:
    ...
    def run_dialog_for_rl(
        self, sys_agent, goal: Goal | None = None, max_turns=20, **kwargs
    ):
        ...
        metrics = {
            ...
            "inform_f1": (
                sess.evaluator.inform_F1()[2]
                if sess.evaluator.inform_F1()[2] is not None
                else 0
            ),
            "goal_satisfaction": sess.evaluator.final_goal_analyze(),
            "total_turns": i + 1,
        }
    ...
    def comprehensive_analyze(self, sys_agent, model_name, total_dialog=100, s=5):
        ...
        for j in tqdm(range(total_dialog), desc="dialogue"):
            ...
            for i in range(40):
                ...
                if sess.sys_agent.state_return() is not None:
                    print(
                        "DST state:",
                        sess.sys_agent.state_return()["dst_state"],
                        file=flog,
                    )
            ...
            if (j + 1) % 100 == 0:
                ...
                logger.info(
                    "emotion reward: %.3f", total_total_emotion_reward / (j + 1)
                )
            ...
            dialog_saver.append_dialog(
                goal_seed,
                self.sess.evaluator.goal,
                task_complete,
                task_success,
                task_succ_strict,
                total_return,
        ...
        try:
            print(
                "percentage of inform violations: %.3f"
                % (
                    sys_agent.policy.fail_info_penalty
                    / float(sys_agent.policy.num_generated_response)
                ),
                file=f,
            )
        except:
